...
def process_carla_scene(scene, data, max_timesteps, scene_config):
    for node_id in pd.unique(data['node_id']):
        ...
        if not np.all(np.diff(node_df['frame_id']) == 1):
            ...
            logging.debug(f"Found an occlusion by node {node_id} in scene {scene.name}.")
            logging.debug(f"List of frame_id is {node_df['frame_id'].values}")
            occl_count = np.diff(node_df['frame_id'])
            logging.debug(f"np.diff() on frame_id is {occl_count}; longest sequence is {sz}")
            if sz < 2:
                continue
            node_df = node_df[s].copy()
            logging.debug(f"Sequence of frame_id is {node_df['frame_id'].values}")
