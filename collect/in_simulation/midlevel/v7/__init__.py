"""
v7 is a modification of v2_2 that does original approach on curved road boundaries
and uses LTV bicycle model linearized around a nominal path.
...
from .util import plot_lcss_prediction
...
class MidlevelAgent(AbstractDataCollector):
    ...
    def make_local_params(self, frame, ovehicles):
        ...
        _, psi_0, _ = carlautil.actor_to_rotation_ndarray(
                self.__ego_vehicle, flip_y=True)
        v_0_mag = np.sqrt(v_0_x**2 + v_0_y**2)
        delta_0 = self.__ego_vehicle.get_control().steer*self.__params.max_delta
    ...
    def do_highlevel_control(self, params, ovehicles):
        ...
        min_u = np.vstack((np.zeros(T), np.full(T, -max_delta))).T.ravel()
